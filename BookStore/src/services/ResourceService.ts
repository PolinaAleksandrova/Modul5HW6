...
export default class DefaultUserService implements ResourceService {
    ...
    public async getById(id: number): Promise<ResourceDto> {
        const result = await this.httpService.send<ResourceResponse>(`resources/${id}`, MethodType.GET);
        return result.data.data;
    }

    public async getByPage(page: number): Promise<ResourcesDto> {
        const result = await this.httpService.send<ResourcesDto>(`resources?page=${page}`, MethodType.GET);
        return {...result.data};
    }
}
